    ...
    try:
        with db_utils.get_engine().connect() as conn:
            if last_time is None:
                ...
                query_obj = text("SELECT * FROM clean_data ORDER BY time ASC LIMIT 1000")
...
def run_pipeline():
    ...
    try:
        ...
        print(f"📊 Processing {len(df)} new rows from clean_data")
        ...
        # -------- PREPARE ALL ROWS FOR INSERT (not just tail(1)) --------
        result_df = df[[
        ...
        print(f"✅ Generated features for {len(result_df)} rows")
...
# -------- MAIN LOOP --------
print("Starting Member4 feature pipeline loop (60s cadence)")
while True:
    try:
        run_pipeline()
    except Exception as e:
