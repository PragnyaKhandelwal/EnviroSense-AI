...
// 🛡️ FULL SAFE DESTRUCTURING
const sensor = snapshot?.sensor ?? {};
const pm25 = sensor?.pm25 ?? 0;
const pm10 = sensor?.pm10 ?? 0;
const cityAvg = sensor?.cityAvg ?? 0;
const delta = sensor?.delta;
const sparkline = sensor?.sparkline ?? [];
const device_label = sensor?.device_label ?? "Unknown Device";
...
return (
<section
className={`relative panel ${aqiPanelClass(level25)} p-5 sm:p-6 overflow-hidden ${
        isLive ? "scanline" : ""
      } h-full`}
> <div className="grid-bg absolute inset-0 opacity-[0.08] pointer-events-none" />
  ...
  <div className="relative flex items-start justify-between gap-3">
    ...
    <div className="text-right">
      ...
      <div className="font-mono text-xs flex items-center gap-1 justify-end">
        {isLive ? (
          <>
            <span className={aqiTextClass(level25)}>●</span>{" "}
            {formatRelative(lastSyncTs)}
          </>
        ) : (
          <>
  ...
  <div className="relative mt-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 border-t border-border pt-3">
    <div className="text-sm">
      ...
      <span className={`font-medium ${aqiTextClass(level25)}`}>
        {aqiLabel(level25)}
      </span>
    </div>
    ...
    <div className="text-xs text-muted-foreground">
      ...
      <span className={`font-medium ${aqiTextClass(cityLevel)}`}>
        {typeof cityAvg === "number" ? cityAvg.toFixed(1) : "—"} (
        {cityLevel === "clean"
          ? "Green"
          : cityLevel === "moderate"
          ? "Yellow"
...
return ( <div> <div className="flex items-center gap-2 text-xs text-muted-foreground">
{label}
  ...
  <div
    className={`mt-1 font-mono text-4xl sm:text-5xl font-semibold tabular-nums ${aqiTextClass(
      level
    )} ${aqiGlowText(level)}`}
  >
